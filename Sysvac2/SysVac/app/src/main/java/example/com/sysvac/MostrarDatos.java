...
*/
         ...
         mDQbdHelper = new DB(getApplicationContext());
...
/// carga las notificaciones en modo interno  con las notificaciones push
    private void loadNotificaciones() {
       ...
       // int[] meses = {0, 2, 4, 6, 12, 15, 18, 48};
        ...
        ArrayList<Integer> mes = mDQbdHelper.jeje(1);
        cursor.moveToFirst();
        Hijo hijo = new Hijo(cursor);
        for (int i = 0; i < mes.size(); i++) {
            String fecha = util.calcularFechaAAplicar(hijo.getFecha_nacimiento(), mes.get(i));
            new Notificacion(getApplicationContext(),
                    util.calcularNotificacion(fecha),
                    hijo.getId_hijo(),
                    hijo.getNombres() + " " + hijo.getApellidos(),
                    mes.get(i));
            System.out.println(fecha);
        }
        cursor.close();
